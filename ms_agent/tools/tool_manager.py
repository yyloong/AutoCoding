    ...
    def __init__(
        ...
        **kwargs,
        ...
        if hasattr(config, "tools") and hasattr(config.tools, "state_transition"):
            self.extra_tools.append(
                State_transition(config, trust_remote_code=self.trust_remote_code)
            )
        if hasattr(config, "tools") and hasattr(config.tools, "execute_shell"):
            self.extra_tools.append(
                execute_shell(config, trust_remote_code=self.trust_remote_code)
            )
        if hasattr(config, "tools") and hasattr(config.tools, "finish"):
            self.extra_tools.append(
                exit_task(config, trust_remote_code=self.trust_remote_code)
            )
        if hasattr(config, "tools") and hasattr(config.tools, "deepresearch"):
            self.extra_tools.append(
                DeepresearchTool(config, trust_remote_code=self.trust_remote_code)
            )
        ...
        if hasattr(config, "tools") and hasattr(config.tools, "kaggle_tools"):
            self.extra_tools.append(
                kaggle_tools(config, trust_remote_code=self.trust_remote_code)
            )
        ...
        if hasattr(config, "tools") and hasattr(config.tools, TOOL_PLUGIN_NAME):
            ...
            for plugin in plugins:
                ...
                module_classes = {
                    name: cls
                    for name, cls in inspect.getmembers(plugin_file, inspect.isclass)
                }
    ...
    async def reindex_tool(self):
        ...
        def extend_tool(tool_ins: ToolBase, server_name: str, tool_list: List[Tool]):
            for tool in tool_list:
                ...
                max_server_len = (
                    MAX_TOOL_NAME_LEN - len(tool["tool_name"]) - len(self.TOOL_SPLITER)
                )
    ...
    async def single_call_tool(self, tool_info: ToolCall):
        ...
        async with self._concurrent_limiter:
            ...
            try:
                ...
                logger.warning(traceback.format_exc())
                return f"Tool calling failed: {brief_info}, details: {str(e)}, check the tool name or parameter"
