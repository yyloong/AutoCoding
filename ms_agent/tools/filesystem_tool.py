...
class FileSystemTool(ToolBase):
    ...
    def __init__(self, config, **kwargs):
        ...
        self.limit_len = getattr(
            getattr(config.tools, "file_system", {}), "limit_len", 1000
        )
        self.allow_read_all_files = getattr(
            getattr(config.tools, "file_system", {}), "allow_read_all_files", False
        )
    ...
    async def read_file(self, paths: list[str]):
        ...
        for path in paths:
            try:
                ...
                is_in_output_dir = (
                    target_path_real.startswith(output_dir_real + os.sep)
                    or target_path_real == output_dir_real
                )
