...
class EvalCallback(Callback):
    ...
    def _parse_e_msg(e):
        stdout = None
        stderr = None
        # try to extract stdout/stderr from exception or result
        if hasattr(e, 'stdout'):
            stdout = e.stdout
    ...
    def check_runtime():
        ...
        env['PYTHONIOENCODING'] = 'utf-8'
        try:
            # cross-platform process kill for node
            if os.name == 'nt':
            ...
            if os.getcwd().endswith('backend'):
                # run dev (non-blocking) with a reasonable timeout
            ...
            output = (result.stdout or '') + '\n' + (result.stderr or '')
        # ensure node processes are killed on Windows too
        if os.name == 'nt':
            os.system('taskkill /F /IM node.exe >nul 2>&1')
        else:
            os.system('pkill -f node')
